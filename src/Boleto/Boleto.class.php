...
abstract class Boleto {
     ...
     * @return Boleto
     */
    public static function load_boleto($arguments = array(), $settings = array()) {
        ...
        if (in_array($bank_code, Boleto::installedPlugins())) {
            ...
            return new $bank_plugin($arguments, $settings);
        }
        else {
    ...
    protected function __construct($arguments, $settings = array()){
        ...
        if (!empty($settings)){
            $this->settingsPropertySetter($settings);
        }
    ...
    protected function barcode(){
        ...
        $black = $this->getImageBase64Content($this->settings['file_location'] . self::IMAGES_FOLDER . $this->settings['bar_code']['black_bar']);
        $white = $this->getImageBase64Content($this->settings['file_location'] . self::IMAGES_FOLDER . $this->settings['bar_code']['white_bar']);
     ...
     * @return string
     */
    private function getImageBase64Content($filepath)
    {
        return sprintf('data:%s;base64,%s', mime_content_type($filepath), base64_encode(file_get_contents($filepath)));
    }
}
